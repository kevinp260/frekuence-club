...
| `npm run test:e2e`                                               | Pending: the post-fix attempt completed its build and production validation, but the browser suite could not launch because `/usr/bin/google-chrome` is unavailable in the runner                              |
